...
#define THIS_SCRIPT "210FTPCalib"
...
bool heartBeatOn    = false;    // heart beat (message) enabled
...
GetFlowWeight(Ptr<QueueDiscItem> item)
{
    ...
    Ptr<ArpQueueDiscItem> arpItem = DynamicCast<ArpQueueDiscItem>(item);
    if (arpItem != nullptr) {
        // treat ARP as from flow 0 and set its weight to 0.05 (very low)
        return 0.05;
    }
...
main(int argc, char* argv[])
{
    ...
    cmd.AddValue("holdTime", "Device queue BQL hold time", holdTime);
    cmd.Parse(argc, argv);
    ...
    if (simParamsPrint) {
        ...
        std::cout << "qdQSize = " << qdQSize << "p" << std::endl;
        std::cout << "devQType = " << devQType << std::endl;
        std::cout << "devQSize = " << devQSize << "p" << std::endl;
        std::cout << "holdTime = " << holdTime << "ms" << std::endl;
    }

    //
    // Global ns-3 configurations
    //
    // Example: Create a packet sink on the star "hub" to receive these packets
    //
    //      uint16_t port = 50000;
    ...
    //
    // By default, the ns3::TcpSocketFactory uses ns-3 TCP. If you want other TCP 
    // implementation, you have to configure ns3::TcpL4Protocol::SocketType like below:
    //
    ...
    //
    // In addition, there are a number of ns3::TcpSocket attributes that can be configured, e.g.,
    // InitialCwnd, SegmentSize, SndBufSize, and RcvBufSize.
    //
    // Also, check ns3::TcpSocketBase, a subclass of ns3::TcpSocket.  It has lots of attributes 
    // too. These inlcude Sack, UseEcn, etc.
    //
    //      Config::SetDefault("ns3::TcpSocketBase::UseEcn", StringValue("On"))
    //
    // Pay attention to ns3::TcpYeah, a high speed TCP congestion control algorithm besides 
    // ns3::TcpHighSpeed.
    //
    ...
    p2pHelper.SetQueue("ns3::DropTailQueue", "MaxSize", StringValue(std::to_string(devQSize)+"p"));

    // TODO: should we do BQL here?????
    ...
    // TODO: note all devices above have a default device queue: should we use BQL?
    ...
    for (uint32_t i = 0; i < nFTPs; i++) {
        ...
        ftpSourceApps.Start(Seconds(0.0001));
        ftpSourceApps.Stop(Seconds(simTime));

        // FTP sink
        Address sinkLocalAddress(InetSocketAddress(Ipv4Address::GetAny(), port));
        PacketSinkHelper ftpSinkHelper("ns3::TcpSocketFactory", sinkLocalAddress);
        ...
        sink1 = DynamicCast<PacketSink>(ftpSinkApps.Get(0));
    }
    ...
    std::cout << "Throughput (Mbps): " << sink1->GetTotalRx() * 8 / simTime / 1e6 << std::endl;
